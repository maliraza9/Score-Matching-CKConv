...
def parse_id(id_, missing_ratio=0.1):
    # data = pd.read_csv("C:/runs/logs/diffmodel_ckconv/data/physio/set-a/{}.txt".format(id_))
    ...
    masks = observed_masks.reshape(-1).copy()  #python list of True of False in 48X35 = (1680,)
    obs_indices = np.where(masks)[0].tolist()   #list of integer indexes python list (1680-nans,) after removing NaNs.
    ...
    observed_values = np.nan_to_num(observed_values) #includes NaN but now converted into NaN=0 or inf= +/- >>> number
...
def get_idlist():
    patient_id = []
    for filename in os.listdir("C:/runs/logs/diffmodel_ckconv/data/physio/set-a"):
...
class Physio_Dataset(Dataset):
    def __init__(self, eval_length=48, use_index_list=None, missing_ratio=0.0, seed=0):
        ...
        self.gt_masks = []
        # path = (
        #     "C:/runs/logs/diffmodel_ckconv/data/physio_missing" + str(missing_ratio) + "_seed" + str(seed) + ".pk"
        # )
        path = (
        ...
        if os.path.isfile(path) == False:  # if datasetfile is none, create
            idlist = get_idlist()         # instead of in path list iteration, do this for all 1024 rows as id_ in idlist for loop
            for id_ in idlist:
                try: # remove try just do normal
                ...
                except Exception as e:  #not useful
                    print(id_, e)
                    continue
            self.observed_values = np.array(self.observed_values) #should be np array already. should be all values of all
                                                                # 1024,51,2
            ...
            tmp_values = self.observed_values.reshape(-1, 35)     #is this for 2 variables? so do
                                                                    # 1024X51, 2
            tmp_masks = self.observed_masks.reshape(-1, 35) # 1024X51, 2

            mean = np.zeros(35) #2 instead 35
            std = np.zeros(35)
            for k in range(35): #2 instead 35
